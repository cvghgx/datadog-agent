...
func Validate(rawPod []byte, ns string, validationType string, v ValidationFunc, dc dynamic.Interface) (bool, error) {
	var pod corev1.Pod
	...
	if err != nil {
		// TODO (wassim): Check telemetry
		metrics.ValidationAttempts.Inc(validationType, metrics.StatusError, strconv.FormatBool(false), err.Error())
		return false, fmt.Errorf("failed to validate pod: %v", err)
	}

	// TODO (wassim): Check telemetry
	metrics.ValidationAttempts.Inc(validationType, metrics.StatusSuccess, strconv.FormatBool(validated), "")
